...
import { GameEntry, NON_THEMATIC_CATEGORY_LABELS, gameKey } from '../utils/spinolocoCatalog';
...
test.describe('spinoloco7545.com — Thumbnails & Categories', () => {
  ...
  for (const { name: viewportName, config: viewportConfig } of VIEWPORTS) {
    test.describe(viewportName, () => {
      ...
      test(
        `Thumbnail check — every Casino & Live Casino game has a real image — ${viewportName}`,
        { tag: ['@provider-launch'] },
        async ({ page }) => {
          ...
          await spinoloco.attachScreenshot(`Thumbnail check — ${viewportName}`);

          if (cappedAny) {
            recordIssue({
              where: `Thumbnail check — ${viewportName}`,
              severity: 'Low',
          ...
          if (broken.length > 0) {
            recordIssue({
              where: `Broken thumbnails — ${viewportName}`,
              severity: 'Medium',
      ...
      test(
        `Category check — every game is assigned at least one category — ${viewportName}`,
        { tag: ['@provider-launch'] },
        async ({ page }) => {
          test.setTimeout(20 * 60_000);
          allure.severity('normal');
          ...
          for (const section of uniqueSections) {
            if (NON_THEMATIC_CATEGORY_LABELS.has(section.label.toUpperCase())) continue;
            try {
          ...
          await spinoloco.attachScreenshot(`Category check — ${viewportName}`);

          if (uniqueSections.length === 0) {
            recordIssue({
              where: `Category discovery — ${viewportName}`,
          ...
          if (brokenSections.length > 0) {
            recordIssue({
              where: `Category listing pages — ${viewportName}`,
              severity: 'Medium',
          ...
          if (uncategorized.length > 0 && uniqueSections.length > 0) {
            recordIssue({
              where: `Uncategorized games — ${viewportName}`,
              severity: 'Medium',
